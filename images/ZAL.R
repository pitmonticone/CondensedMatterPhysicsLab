...
sd1 <- sd(d1)/(sqrt(length(d1))) #STD ERROR
...
D2m <- mean(D2)
dD2 <- d2m/D2m
sdD2 <- sqrt((sd2/D2m)^2+((sD2*d2m)/(D2m^2))^2) # 0.069 pm 0.012
...
D5 <- c(A1[3]-A1[1],A1[4]-A1[2],A2[3]-A2[1],A2[4]-A2[2],A3[3]-A3[1],A3[4]-A3[2],A3[5]-A3[3],A3[6]-A3[4])
sd5 <- sd(d5)/(sqrt(length(d5)))
sD5 <- sd(D5)/(sqrt(length(D5)))
d5m <- mean(d5)
D5m <- mean(D5)
dD5 <- d5m/D5m
sdD5 <- sqrt((sd5/D5m)^2+((sD5*d5m)/(D5m^2))^2) # 0.147 pm 0.022
...
D6m <- mean(D6)
dD6 <- d6m/D6m
sdD6 <- sqrt((sd6/D6m)^2+((sD6*d6m)/(D6m^2))^2) # 0.130 pm 0.022
